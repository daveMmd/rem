...
def split_set(entries, target_size):
	...
	error = int(total_length / 17 * 0.5)
	print("error:", error)
	total_size = 0
	for i in range(target_size):
		this_size = int(total_length/17) + random.randint(-error, error)
		E_set.append(entries[total_size : total_size+this_size])
...
'''
expand to entries , then split into multiple Es.
'''
def main2():
	fname = "./ClassBench/m_acl_5k"
	rules = get_rules(fname)
	entries = get_entries(rules)
	E_set = split_set(entries, 17)
	for i in range(len(E_set)):
		writeE(i+1, E_set[i])
